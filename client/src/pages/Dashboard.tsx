...
export default function Dashboard() {
  ...
  const { data: todayPomodoros = [], isLoading: isLoadingPomodoro } = useQuery({
    queryKey: ["/api/pomodoro/today"],
  });

  const isLoading = isLoadingHabits || isLoadingLogs || isLoadingPomodoro;
        ...
        {isLoading ? (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-card rounded-3xl p-5 h-32 animate-pulse" />
              ))}
            </div>
            {/* Habits Skeleton */}
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-card rounded-3xl p-6 h-24 animate-pulse" />
              ))}
            </div>
          </div>
        ) : (
